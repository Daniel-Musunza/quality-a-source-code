                ...
                <div class="card">
                    ...
                    <div class="table-responsive">
                        <table width="100%">
                            ...
                            <tbody>
                                <tr v-for="(order, index) in revisions" :key="order.id">
                                        <td>{{index + 1}}</td>
                                        <td>{{order.id}}</td>
...
export default {
    ...
    async created() {
        ...
        const orderRef = collection(db, 'revisions');
        const snapshot = await getDocs(orderRef);
        const revisions = snapshot.docs.map(doc => doc.data());

        const promises = revisions.map(async order => {
            if (order.freelancer) {
